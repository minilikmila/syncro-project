       ...
       <v-card>
         ...
         <v-divider class="red mb-4"></v-divider>
            <v-form ref="form">
             ...
             <v-card-text>
              <v-text-field label="First Name" outlined v-model="accountData.F_name"></v-text-field>
              <v-text-field label="Last Name" outlined v-model="accountData.L_name"></v-text-field>
              <v-text-field label="Username" outlined v-model="accountData.username"
                            @focus="unique=false"></v-text-field>
             </v-card-text>
           </v-window-item>
             ...
             <v-card-text>
              <v-text-field label="E-mail" :rules="emailRule" outlined v-model="accountData.email"
                 ...
                 <v-file-input
                              ...
                              outlined
                              v-model="accountData.image"></v-file-input>
              ...
              <v-card-text>
                <v-textarea
                 ...
                 <v-text-field label="Password" outlined
                             :rules="passRule"  v-model="accountData.password" @focus="passwordWarn= false"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    addUserAccount(){
      let data = {
         ...
         imageUrl: this.accountData.image.name,
      }
    let social = {
        social_media: {
     ...
     if (this.accountData.password!='') {
      ...
      // console.log(this.accountData.image.name)
    },
    ...
    directLogin(){
     ...
     api.login(userData).then(response => {
        ...
        if(this.accountData.image != 0){
          console.log("Image");
          api.uploadImage(this.accountData.image);
          this.$store.commit('setImageUrl', this.accountData.image.name);
        }
  ...
  computed: {
    registerOn(){
      let x = true;
      if(this.accountData.username !='' && this.accountData.username !='' && this.checkbox &&
         this.accountData.email !='' && this.accountData.password.length > 5)
      {   return !x   }
      else
      return x
    },
    currentTitle(){
      switch(this.step){
          ...
          default: return 'Final data fields'
      }
    }
  }
}
